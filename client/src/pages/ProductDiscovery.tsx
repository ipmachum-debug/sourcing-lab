...
export default function ProductDiscovery() {
  ...
  const createJob = trpc.extension.createJob.useMutation({
    onSuccess: data => {
      toast.success(`"${keyword}" 분석 작업이 생성되었습니다. 확장 프로그램에서 크롤링을 시작하세요.`);
      setKeyword("");
      jobs.refetch();
      overview.refetch();
    },
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              키워드 입력 → 검색 크롤링 → 상세 크롤링 → AI 분석 → 추적/거절
            </p>
          </div>
          <Button
            variant="outline"
            size="sm"
            onClick={() => { jobs.refetch(); overview.refetch(); pendingProducts.refetch(); }}
          >
        ...
        {ov && (
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            <Card><CardContent className="p-3 text-center">
              ...
              <p className="text-xl font-bold">{formatNum(ov.totalJobs)}</p>
            </CardContent></Card>
            <Card><CardContent className="p-3 text-center">
            ...
            <Card><CardContent className="p-3 text-center">
              ...
              <p className="text-xl font-bold text-purple-500">{formatNum(ov.tracked)}</p>
            </CardContent></Card>
          </div>
        )}
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-xs text-muted-foreground mt-2">
              작업 생성 후 확장 프로그램 사이드패널에서 "발견 크롤링 시작" 버튼을 눌러주세요.
            </p>
          </CardContent>
        </Card>
        ...
        <div>
          ...
          {jobList.length === 0 ? (
            <Card><CardContent className="p-8 text-center text-muted-foreground">
              아직 분석 작업이 없습니다. 키워드를 입력하고 분석을 시작하세요.
            </CardContent></Card>
          ) : (
                ...
                >
                  <CardContent className="p-3 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2 text-xs text-muted-foreground">
                      {j.detailCrawledCount > 0 && (
                        <span>상세 {j.detailCrawledCount}개</span>
                      )}
                      {j.createdAt && (
                        <span>{String(j.createdAt).slice(5, 16)}</span>
                      )}
        ...
        {selectedJob && detail && (
          <div>
            ...
            {detail.job.errorMessage && (
              <Card className="border-red-300 bg-red-50 dark:bg-red-900/20 mb-3">
