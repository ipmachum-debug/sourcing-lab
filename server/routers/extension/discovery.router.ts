/**
 * Extension Sub-Router: AI 제품 발견 (Product Discovery)
 *
 * 파이프라인:
 * 1. 유저 키워드 입력 → discovery job 생성 (pending)
 * 2. 확장 프로그램이 pending job 폴링 → 검색 크롤링
 * 3. 서버에서 1차 필터링 → 상세 크롤링 대상 선정
 * 4. 확장이 상세 페이지 크롤링 → 결과 전송
 * 5. 서버 AI 분석 → 추천 제품 생성
 * 6. 유저가 추적/거절 결정
 */
import { z } from "zod";
